...
const AnalisisFoliar = () => {
  ...
  const [pendingData, setPendingData] = useState<Partial<ILeafAnalysisRecord> | null>(null);
    ...
    try {
      ...
      if (workAssociationData.associateWork) {
        ...
        handleResponseWithFallback(
          result,
          'creation',
          'LEAF_ANALYSIS_RECORD',
          "Análisis foliar creado correctamente"
        );
      } else {
      ...
      console.error("Error creating leaf analysis with work association:", error);

      handleErrorWithEnhancedFormat(
        error,
        'creation',
        'LEAF_ANALYSIS_RECORD',
        "No se pudo crear el análisis foliar"
      );
    }
  };

  // Create entity without associated work
  const createEntityWithoutWork = async (data: Partial<ILeafAnalysisRecord>) => {
    await leafAnalysisService.createLeafAnalysis(data);
  };

  // Create entity with associated work
  const createEntityWithWork = async (
    entityData: Partial<ILeafAnalysisRecord>,
  ...
  const handleUpdateLeafAnalysis = async (id: string | number, data: Partial<ILeafAnalysisRecord>) => {
    try {
      const result = await leafAnalysisService.updateLeafAnalysis(id, data);
      ...
      handleResponseWithFallback(
        ...
        'update',
        'LEAF_ANALYSIS_RECORD',
        "Análisis foliar actualizado correctamente"
      );
      ...
      handleErrorWithEnhancedFormat(
        ...
        'update',
        'LEAF_ANALYSIS_RECORD',
        "No se pudo actualizar el análisis foliar"
      );
    }
  };
      ...
      <Dialog open={showConfirmation} onOpenChange={setShowConfirmation}>
        <DialogContent className="w-[95vw] max-w-[95vw]">
          <DialogHeader>
            ...
            <DialogDescription>
              ¿Está seguro que desea crear el análisis foliar sin asociar un trabajo?
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          {showWorkWizard && pendingData && (
            <WorkAssociationWizard
              entityType="analisisFoliar"
              entityData={{
                id: "new-leaf-analysis"
              }}
