...
const CalibrarAspersion = () => {
  ...
  const [pendingData, setPendingData] = useState<Partial<ICalibrateSprinkler> | null>(null);
  ...
  // Fetch calibraciones on component mount and when propertyId changes
  useEffect(() => {
    if (propertyId) {
      fetchCalibraciones();
      loadWorkWizardData();
    ...
    try {
      ...
      if (workAssociationData.associateWork) {
        ...
        handleResponseWithFallback(
          ...
          'creation',
          'CALIBRATE_SPRINKLER',
          "Calibración de aspersión creada correctamente"
        );
      } else {
      ...
      handleErrorWithEnhancedFormat(
        ...
        'creation',
        'CALIBRATE_SPRINKLER',
        "No se pudo crear la calibración de aspersión"
      );
    }
  };

  // Create calibration without associated work
  const createEntityWithoutWork = async (data: Partial<ICalibrateSprinkler>) => {
    await calibrateSprinklerService.createCalibrateSprinkler(data);
  };

  // Create calibration with associated work
  const createEntityWithWork = async (
    entityData: Partial<ICalibrateSprinkler>,
  ...
  const handleUpdateCalibracion = async (id: string | number, data: Partial<ICalibrateSprinkler>) => {
    try {
      const result = await calibrateSprinklerService.updateCalibrateSprinkler(id, data);
      ...
      handleResponseWithFallback(
        ...
        'update',
        'CALIBRATE_SPRINKLER',
        "Calibración de aspersión actualizada correctamente"
      );
      ...
      handleErrorWithEnhancedFormat(
        ...
        'update',
        'CALIBRATE_SPRINKLER',
        "No se pudo actualizar la calibración de aspersión"
      );
    }
  };
      ...
      <Dialog open={showConfirmation} onOpenChange={setShowConfirmation}>
        <DialogContent className="w-[95vw] max-w-[95vw]">
          <DialogHeader>
            ...
            <DialogDescription>
              ¿Está seguro que desea crear la calibración de aspersión sin asociar un trabajo?
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          {showWorkWizard && pendingData && (
            <WorkAssociationWizard
              entityType="calibrarAspersion"
              entityData={{
                id: "new-calibrate-sprinkler"
              }}
