...
const LavadoManos = () => {
  ...
  const [pendingData, setPendingData] = useState<Partial<IHandWashingRecord> | null>(null);
    ...
    try {
      ...
      if (workAssociationData.associateWork) {
        ...
        handleResponseWithFallback(
          ...
          'creation',
          'HAND_WASHING_RECORD',
          "Registro de lavado de manos creado correctamente"
        );
      } else {
      ...
      console.error("Error creating hand washing record with work association:", error);

      handleErrorWithEnhancedFormat(
        error,
        'creation',
        'HAND_WASHING_RECORD',
        "No se pudo crear el registro de lavado de manos"
      );
    }
  };

  // Create record without associated work
  const createEntityWithoutWork = async (data: Partial<IHandWashingRecord>) => {
    await handWashingService.createHandWashingRecord(data);
  };

  // Create record with associated work
  const createEntityWithWork = async (
    recordData: Partial<IHandWashingRecord>,
  ...
  const handleUpdateRecord = async (id: string | number, data: Partial<IHandWashingRecord>) => {
    try {
      const result = await handWashingService.updateHandWashingRecord(id, data);
      ...
      handleResponseWithFallback(
        ...
        'update',
        'HAND_WASHING_RECORD',
        "Registro de lavado de manos actualizado correctamente"
      );
      ...
      handleErrorWithEnhancedFormat(
        ...
        'update',
        'HAND_WASHING_RECORD',
        "No se pudo actualizar el registro de lavado de manos"
      );
    }
  };
      ...
      <Dialog open={showConfirmation} onOpenChange={setShowConfirmation}>
        <DialogContent className="w-[95vw] max-w-[95vw]">
          <DialogHeader>
            ...
            <DialogDescription>
              ¿Está seguro que desea crear el registro de lavado de manos sin asociar un trabajo?
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          {showWorkWizard && pendingData && (
            <WorkAssociationWizard
              entityType="lavadoManos"
              entityData={{
                id: "new-hand-washing-record"
              }}
