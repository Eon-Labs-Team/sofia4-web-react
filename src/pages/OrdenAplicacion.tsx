...
import { z } from "zod";
import { IWork } from "@eon-lib/eon-mongoose/types";
import { IWorkers } from "@eon-lib/eon-mongoose/types";
import { IWorkerList } from "@eon-lib/eon-mongoose/types";
import { IMachinery } from "@eon-lib/eon-mongoose/types";
import { IMachineryList } from "@eon-lib/eon-mongoose/types";
import { IProducts } from "@eon-lib/eon-mongoose/types";
import { IProductCategory } from "@eon-lib/eon-mongoose/types";
// @ts-ignore
import { IWarehouseProduct } from "@eon-lib/eon-mongoose/types";
import { ITaskType, TaskTypeWorkType } from "@eon-lib/eon-mongoose/types";
import { ITask } from "@eon-lib/eon-mongoose/types";
import { IWeatherCondition } from "@eon-lib/eon-mongoose/types";
import { IWindCondition } from "@eon-lib/eon-mongoose/types";
import { ICropType } from "@eon-lib/eon-mongoose/types";
import { IVarietyType } from "@eon-lib/eon-mongoose/types";
...
const OrdenAplicacion = () => {
  ...
  const ordersGridcolumns: Column[] = [
    ...
    },
    {
      ...
      accessor: "specificData.appliedHectares",
      visible: true,
      sortable: true,
      render: (value: any, row: any) => row.specificData?.appliedHectares || 0
    },
    {
      id: "generalObjective",
      header: "Objetivo General",
      accessor: "specificData.generalObjective",
      visible: true,
      sortable: true,
      render: (value: any, row: any) => row.specificData?.generalObjective || ''
    },
    {
      id: "workState",
      header: "Estado",
  ...
  const fetchTaskTypes = async () => {
    try {
      ...
      const applicationTaskTypes = data.filter((taskType: ITaskType) => taskType.workType === ('A' as TaskTypeWorkType));
  ...
  //       formSetValue('specificData.applicators.0.userId', workerId);
  //       formSetValue('specificData.applicators.0.name', workerName);
  //     }
              ...
              if (section.id === "orden-info-responsibles") {
                return {
                  ...
                  fields: section.fields.map(field => {
                    ...
                    if (field.id === "specificData.applicators.0.userId") {
                      return { ...field, options: workerOptions };
                    }
